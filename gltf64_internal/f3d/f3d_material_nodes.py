...
class F3DNodeA(ShaderNode):
    ...
    def update_F3DNodeA(self, context):
        ...
        if out.is_linked:
            for link in out.links:
                if link.is_valid:
                    link.to_socket.default_value = CCMUXDict[self.inA]
...
class F3DNodeB(ShaderNode):
    ...
    def update_F3DNodeB(self, context):
        ...
        if out.is_linked:
            for link in out.links:
                if link.is_valid:
                    link.to_socket.default_value = CCMUXDict[self.inB]
...
class F3DNodeC(ShaderNode):
    ...
    def update_F3DNodeC(self, context):
        ...
        if out.is_linked:
            for link in out.links:
                if link.is_valid:
                    link.to_socket.default_value = CCMUXDict[self.inC]
...
class F3DNodeD(ShaderNode):
    ...
    def update_F3DNodeD(self, context):
        ...
        if out.is_linked:
            for link in out.links:
                if link.is_valid:
                    link.to_socket.default_value = CCMUXDict[self.inD]
...
class F3DNodeA_alpha(ShaderNode):
    ...
    def update_F3DNodeA_alpha(self, context):
        ...
        if out.is_linked:
            for link in out.links:
                if link.is_valid:
                    link.to_socket.default_value = ACMUXDict[self.inA_alpha]
...
class F3DNodeB_alpha(ShaderNode):
    ...
    def update_F3DNodeB_alpha(self, context):
        ...
        if out.is_linked:
            for link in out.links:
                if link.is_valid:
                    link.to_socket.default_value = ACMUXDict[self.inB_alpha]
...
class F3DNodeC_alpha(ShaderNode):
    ...
    def update_F3DNodeC_alpha(self, context):
        ...
        if out.is_linked:
            for link in out.links:
                if link.is_valid:
                    link.to_socket.default_value = ACMUXDict[self.inC_alpha]
...
class F3DNodeD_alpha(ShaderNode):
    ...
    def update_F3DNodeD_alpha(self, context):
        ...
        if out.is_linked:
            for link in out.links:
                if link.is_valid:
                    link.to_socket.default_value = ACMUXDict[self.inD_alpha]
